(function(kg) {
    ...
    var Grid = function(definition) {
        ...
        self.recordTemplate = ko.computed(
            function() {
                var template = 'kg-' + this.model().toLowerCase() + '-template';
                if (this.readonly()) {
                    template += '-pick';
                } 
                return template;
        ...
        self.records = ko.computed(
            {
                ...
                "write": function (definitions) {
                    ...
                    ko.utils.arrayForEach(
                        definitions,
                        function(recordData) {
                            var record = kg.record(recordData, modelKey);
                            record.page(initialOffset + self._records().length);
                            record.dataLanguage(dataLanguage);
